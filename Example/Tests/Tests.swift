...
extension Tests {
    ...
    func testStop_isAtomicAgainstConcurrentTaskAdds() {
        ...
        manager.addTaskAfterInit(taskName: "after") { $0() }
        XCTAssertEqual(manager.queuedTaskCount, 0, "a stopped manager must keep refusing")
    }
...
extension Tests {
  ...
  func testE2E_404OnDeviceUpdate_doesNotStopTheSdk() {
    ...
    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "after-404") { $0() }
    XCTAssertGreaterThan(FlareLaneTaskManager.shared.queuedTaskCount, 0,
  ...
  func testE2E_410FromOtherEndpoint_doesNotStopTheSdk() {
    ...
    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "after-iam-410") { $0() }
    XCTAssertGreaterThan(FlareLaneTaskManager.shared.queuedTaskCount, 0,
  ...
  func testE2E_410OnDeviceCreate_stopsTheSdk() {
    ...
    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "after") { $0() }
    XCTAssertEqual(FlareLaneTaskManager.shared.queuedTaskCount, 0,
  ...
  func testE2E_stoppedSdkStillAnswersSubscribeAndUnsubscribe() {
    ...
    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "probe") { $0() }
    XCTAssertEqual(FlareLaneTaskManager.shared.queuedTaskCount, 0,
  ...
  func testE2E_resetRecoversAfterStop() {
    ...
    updateDeviceAndWait()
    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "stopped") { $0() }
    XCTAssertEqual(FlareLaneTaskManager.shared.queuedTaskCount, 0)

    FlareLaneTaskManager.shared.reset()

    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "after-reset") { $0() }
    XCTAssertGreaterThan(FlareLaneTaskManager.shared.queuedTaskCount, 0,
  ...
  func testE2E_tasksThatNeverRunStillAnswerTheirCaller() {
    startE2E()
    defer { endE2E() }
    ...
    let queuedThenCancelled = expectation(description: "queued task answers after stop()")
    manager.addTaskAfterInit(taskName: "queued", onCancelled: { queuedThenCancelled.fulfill() }) { $0() }
    XCTAssertGreaterThan(manager.queuedTaskCount, 0)

    manager.stop()

    let refusedAfterStop = expectation(description: "refused task answers")
    manager.addTaskAfterInit(taskName: "refused", onCancelled: { refusedAfterStop.fulfill() }) { $0() }

    wait(for: [queuedThenCancelled, refusedAfterStop], timeout: 5)
  }

  /// resetDevice() discards pending tasks too, so their callers are owed the same answer. A
  /// suspended queue would keep holding the cancelled operations and swallow this.
  func testE2E_resetAlsoAnswersTheTasksItDiscards() {
    startE2E()
    defer { endE2E() }
    ...
    let answered = expectation(description: "reset answers the discarded task")
    manager.addTaskAfterInit(taskName: "queued", onCancelled: { answered.fulfill() }) { $0() }
    XCTAssertGreaterThan(manager.queuedTaskCount, 0)
  ...
  /// A task that actually runs must not also be cancelled — the bridges are single-shot.
  func testE2E_aTaskThatRunsIsNeverAlsoCancelled() {
    startE2E()
    defer { endE2E() }
    ...
    var events: [String] = []
    let lock = NSLock()
    let ran = expectation(description: "task runs")

    manager.addTaskAfterInit(taskName: "runs", onCancelled: {
      lock.lock(); events.append("cancelled"); lock.unlock()
    }) { completion in
      lock.lock(); events.append("ran"); lock.unlock()
      completion()
      ran.fulfill()
    }
    wait(for: [ran], timeout: 5)
  ...
  func testE2E_harnessSupportsPerCallResponseSequences() {
    ...
    FlareLaneTaskManager.shared.addTaskAfterInit(taskName: "still-running") { $0() }
    XCTAssertGreaterThan(FlareLaneTaskManager.shared.queuedTaskCount, 0,
